...
async function extractNestCredentials() {
  console.log('🚀 Starting Nest authentication credential extractor...\n');
  ...
  page.on('response', async response => {
    ...
    if (url.includes('iframerpc') && url.includes('issueToken')) {
      try {
        console.log('✅ Found iframerpc call with issueToken!');
        ...
        console.log('📋 Issue Token URL captured\n');
      } catch (error) {
    ...
    if (url.includes('oauth2/iframe') && !foundOauth2Iframe) {
      ...
      console.log('✅ Found oauth2/iframe call - will capture cookies after auth completes\n');
    }
  });
  ...
  try {
    ...
    console.log('⏸️  PLEASE COMPLETE THE FOLLOWING STEPS:');
    ...
    if (issueToken) {
      console.log('\n🍪 Capturing cookies from browser context...');
      ...
      if (googleCookies) {
        ...
        console.log('✅ Captured', allCookies.filter(c => c.domain.includes('google.com')).length, 'Google cookies');
        console.log('   Total cookie string length:', googleCookies.length, 'characters\n');
      } else {
        console.log('⚠️  No Google cookies found\n');
      }
    }

    if (issueToken && cookies) {
      console.log('\n🎉 ✅ SUCCESS! Both credentials captured!\n');
      ...
      const outputFile = '/Users/jlaska/Projects/homelab/scripts/nest-credentials.json';
